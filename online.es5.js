/* ---- ES5 polyfills (Promise + Array.find) ---- */
(function(){'use strict';
  if(!Array.prototype.find){
    Object.defineProperty(Array.prototype,'find',{value:function(predicate){
      ...
      for(var i=0;i<len;i++){var v=o[i]; if(predicate.call(thisArg,v,i,o))return v;}
      return void 0;
  ...
  if(typeof Promise!=='function'){
    (function(){
      ...
      function resolvePromise(p,x,resolve,reject){
        if(p===x)return reject(new TypeError('Promise cycle'));
        var then,called=false;
        if(x!==null&&(typeof x==='object'||typeof x==='function')){
          try{
            then=x.then;
            if(isFunc(then)){
              then.call(x,function(y){if(called)return;called=true;resolvePromise(p,y,resolve,reject);},
                            function(r){if(called)return;called=true;reject(r);});
            } else resolve(x);
          }catch(e){ if(called)return; called=true; reject(e); }
        } else resolve(x);
      }
      function MyPromise(executor){
        if(!isFunc(executor))throw new TypeError('executor must be a function');
        var self=this; self._state=0; self._value=void 0; self._handlers=[];
        function fulfill(v){ if(self._state!==0)return; self._state=1; self._value=v; finale(); }
        function reject(r){ if(self._state!==0)return; self._state=2; self._value=r; finale(); }
        function finale(){ asap(function(){ for(var i=0;i<self._handlers.length;i++)handle(self,self._handlers[i]); self._handlers=[]; }); }
        function handle(self,h){
          if(self._state===0){ self._handlers.push(h); return; }
          asap(function(){
            var cb=self._state===1?h.onFulfilled:h.onRejected;
            if(!isFunc(cb)){ (self._state===1?h.resolve:h.reject)(self._value); return; }
            try{ var ret=cb(self._value); h.resolve(ret); }catch(e){ h.reject(e); }
          });
        }
        self.then=function(onFulfilled,onRejected){
          return new MyPromise(function(resolve,reject){ handle(self,{onFulfilled:onFulfilled,onRejected:onRejected,resolve:resolve,reject:reject}); });
        };
        self['catch']=function(onRejected){ return self.then(null,onRejected); };
        try{ executor(fulfill,reject); }catch(e){ reject(e); }
      }
      MyPromise.resolve=function(v){ return new MyPromise(function(res){ res(v); }); };
      MyPromise.reject=function(r){ return new MyPromise(function(_,rej){ rej(r); }); };
      MyPromise.all=function(arr){
        return new MyPromise(function(resolve,reject){
          if(!arr||typeof arr.length==='undefined')return reject(new TypeError('Expected array-like'));
          var out=new Array(arr.length), remaining=arr.length;
          if(remaining===0)return resolve([]);
          for(var i=0;i<arr.length;i++)(function(i){
            MyPromise.resolve(arr[i]).then(function(v){ out[i]=v; if(--remaining===0)resolve(out); },reject);
          })(i);
        });
      };
      MyPromise.race=function(arr){
        return new MyPromise(function(resolve,reject){
          for(var i=0;i<arr.length;i++) MyPromise.resolve(arr[i]).then(resolve,reject);
        });
      };
  ...
  (function(){
    function toHttp(u){
      ...
      if(u.indexOf('https://')===0)return 'http://'+u.slice(8);
      if(u.indexOf('//')===0)return 'http:'+u;
      return u;
    }
    if(typeof XMLHttpRequest!=='undefined'){
      var _open=XMLHttpRequest.prototype.open;
      XMLHttpRequest.prototype.open=function(m,u,a,u1,u2){ try{u=toHttp(u);}catch(e){} return _open.call(this,m,u,a,u1,u2); };
    }
    if(window.Lampa && Lampa.Utils && Lampa.Utils.putScript){
      var _put=Lampa.Utils.putScript;
      Lampa.Utils.putScript=function(arr,ok,cache,ready,uniq){
        try{ for(var i=0;i<arr.length;i++)arr[i]=toHttp(arr[i]); }catch(e){}
        return _put.call(Lampa.Utils,arr,ok,cache,ready,uniq);
      };
    }
  })();
})(); 
...
(function() {
  ...
  var hostkey = "http://krojiuk.online"'.replace('http://', '').replace('http://', '');
  ...
  if (!window.rch_nws || !window.rch_nws[hostkey]) {
    ...
    window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
      if (window.rch_nws[hostkey].type == undefined) {
        ...
        if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
          ...
          net.silent("http://krojiuk.online"'.indexOf(location.host) >= 0 ? 'http://github.com/' : host + '/cors/check', function () {
            check(true);
          }, function () {
            check(false);
          }, false, {
    ...
    window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
      window.rch_nws[hostkey].typeInvoke("http://krojiuk.online"', function () {
              ...
              new Response(compressedStream).arrayBuffer()
                .then(function (compressedBuffer) {
                  ...
                  if (compressedArray.length > html.length) {
                    ...
                    $.ajax({
                      url: "http://krojiuk.online"rch/gzresult?id=' + rchId,
                      type: 'POST',
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      Lampa.Utils.putScript(["http://krojiuk.online"js/nws-client-es5.js"], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  function component(object) {
    ...
    if (balansers_with_search == undefined) {
      ...
      network.silent(account("http://krojiuk.online"lite/withsearch'), function(json) {
  ...
  if (false) {
    if (Platform.is('browser') && location.host.indexOf("127.0.0.1") !== -1) {
      ...
      $.get("http://krojiuk.online"player-inner/' + element.url);
      return;
    }
